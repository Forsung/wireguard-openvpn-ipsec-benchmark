...
if [[ -z "$ROLE" ]]; then
  echo "Usage: $0 <wsl|pi> [wifi-band] [stage]"
  exit 1
fi
...
run_stage() {
  ...
  case "$stage" in
    ...
    mtu)
      for r in $(seq 1 "$REPEATS"); do
        echo "[mtu1480] WireGuard run $r"
        run_case mtu wireguard "$WG_SERVER_IP" "$r" --mtu 1480 --capture

        echo "[mtu1480] OpenVPN run $r"
        run_case mtu openvpn "$OVPN_SERVER_IP" "$r" --client-conf "$OVPN_CLIENT_CONF" --mtu 1480 --capture

        echo "[mtu1480] IPsec run $r"
        run_case mtu ipsec "$IPSEC_SERVER_IP" "$r" --mtu 1480 --capture
      done
      ;;
